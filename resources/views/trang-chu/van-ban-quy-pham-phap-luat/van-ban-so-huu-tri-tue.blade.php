...
<section class="col">
                <div class="container mt-5">
                    ...
                    {{-- Tiêu đề --}}
                    <h2 class="text-center fw-bold text-dark mb-3">VĂN BẢN SỞ HỮU TRÍ TUỆ QUY PHẠM PHÁP LUẬT</h2>
                    ...
                    <div class="table-responsive"> {{-- Thêm class này để bảng cuộn ngang trên màn hình nhỏ --}}
                        <table class="table mt-4 border-top border-bottom" style="width: 100%; border-color: #00008B;">
                            ...
                            <tbody>
                                @forelse ($thongbao as $document)
                                    <tr style="border-bottom: 1px solid #dee2e6;">
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $document->title }}</td>
                                        <td class="text-end">
                                            <a href="{{ route('trang-chu.van-ban-quy-pham-phap-luat.show-vanbansohuutritue', $document->_id) }}"                       
