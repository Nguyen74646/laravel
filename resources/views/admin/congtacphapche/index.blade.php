...
@section("title", "Comgtacphapche")

@section('content')
<div class="container mt-4">
    <h3 class="mb-4">Danh sách sự kiện hoạt động</h3>
    ...
    @if(Auth::check() && Auth::user()->role == 'admin')
        <a href="{{ route('admin.congtacphapche.create') }}" class="btn btn-primary mb-3">➕ Thêm hồ sơ mới</a>
    @elseif(Auth::check() && Auth::user()->role == 'user')
        <a href="{{ route('user.congtacphapche.create') }}" class="btn btn-primary mb-3">➕ Thêm hồ sơ mới</a>
    @endif
    <!---------------------------------------------------------------------------------->
    {{-- Nút thêm mới --}}
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group" role="group">
                                <button class="btn btn-sm btn-info" title="Xem" data-bs-toggle="modal" data-bs-target="#viewModal-{{ $hoso->id }}">👁️</button> 
                                <!-- Nút Xem -->
                                <!---------------------------------------------------------------------------------->
                                @if(Auth::check() && Auth::user()->role == 'admin')
                                <a href="{{ route('admin.congtacphapche.edit', $hoso->id) }}" class="btn btn-sm btn-primary" title="Sửa">✏️</a>
                                @elseif(Auth::check() && Auth::user()->role == 'user')
                                <a href="{{ route('user.congtacphapche.edit', $hoso->id) }}" class="btn btn-sm btn-primary" title="Sửa">✏️</a>
                                @endif
                                <!---------------------------------------------------------------------------------->
                                ...
                                <!---------------------------------------------------------------------------------->
                                @if(Auth::check() && Auth::user()->role == 'admin')
                                    <form action="{{ route('admin.congtacphapche.destroy', $hoso->_id) }}"  method="POST" style="display: inline;">
                                @elseif(Auth::check() && Auth::user()->role == 'user')
                                 <form action="{{ route('user.congtacphapche.destroy', $hoso->_id) }}"  method="POST" style="display: inline;">
                                @endif
                                <!---------------------------------------------------------------------------------->
                                     @csrf
                                    @method('DELETE')
...
@endforeach
    ...
    {{-- Nút quay lại --}}
    <a href="{{ url('/user') }}" class="btn btn-outline-primary mb-3">
        ← Quay lại admin
    </a>
{{-- Scripts --}}
@push('scripts')
