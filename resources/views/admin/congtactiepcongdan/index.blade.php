...
<div class="container mt-4">
    <h3 class="mb-4">Danh sách công tác tiếp công dân mới</h3>
    ...
    @if(Auth::check() && Auth::user()->role == 'admin')
      <a href="{{ route('admin.congtactiepcongdan.create') }}" class="btn btn-primary mb-3">➕ Thêm công tác mới</a>
    @elseif(Auth::check() && Auth::user()->role == 'user')
        <a href="{{ route('user.congtactiepcongdan.create') }}" class="btn btn-primary mb-3">➕ Thêm công tác mới</a>
    @endif
    <!---------------------------------------------------------------------------------->
    {{-- Bảng danh sách --}}
                        ...
                        <td>
                            <div class="btn-group" role="group">
                                <button class="btn btn-sm btn-info" title="Xem" data-bs-toggle="modal" data-bs-target="#viewModal-{{ $hoso->id }}">👁️</button> <!-- Nút Xem -->
                                    ...
                                    <!---------------------------------------------------------------------------------->
                                    @if(Auth::check() && Auth::user()->role == 'admin')
                                        <a href="{{ route('admin.congtactiepcongdan.edit', $hoso->id) }}" class="btn btn-sm btn-primary" title="Sửa">✏️</a>
                                    @elseif(Auth::check() && Auth::user()->role == 'user')
                                        <a href="{{ route('user.congtactiepcongdan.edit', $hoso->id) }}" class="btn btn-sm btn-primary" title="Sửa">✏️</a>
                                    @endif
                                    <!---------------------------------------------------------------------------------->
                                    
                                    <!---------------------------------------------------------------------------------->
                                    @if(Auth::check() && Auth::user()->role == 'admin')
                                    <form action="{{ route('admin.congtactiepcongdan.destroy', $hoso->_id) }}"  method="POST" style="display: inline;">
                                    @elseif(Auth::check() && Auth::user()->role == 'user')
                                    <form action="{{ route('user.congtactiepcongdan.destroy', $hoso->_id) }}"  method="POST" style="display: inline;">
                                    @endif
                                    <!---------------------------------------------------------------------------------->
                                    @csrf
                                    @method('DELETE')
