...
<div class="container mt-4">
    ...
    {{ route('admin.sukien.create') }}
     @elseif(Auth::check() && Auth::user()->role == 'user')
      {{ route('user.sukien.create') }}
        @endif
    " class="btn btn-primary mb-3">➕ Thêm hồ sơ mới</a>
    ...
    <!---------------------------------------------------------------------------------->
                                <a href="{{ route(Auth::user()->role . '.sukien.edit', $hoso->id) }}" class="btn btn-sm btn-primary" title="Sửa">✏️</a>
                                <form action="{{ route(Auth::user()->role . '.sukien.destroy', $hoso->_id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-sm btn-danger" onclick="return confirm('Bạn Có Chắc Muốn Xóa Người Dùng Này Không?')" type="submit">🗑️</button>
                                </form>
